...
void MPU6050_Initialization(void)
{
	...
	else
	{
		...
		char msg[60];
		sprintf(msg, "MPU6050 who_am_i : 0x%02x should be 0x68\n", who_am_i);
		HAL_UART_Transmit(&huart2, (uint8_t *)msg, strlen(msg), 100);
		while(1)
		{
			HAL_UART_Transmit(&huart2, (uint8_t *)"who am i error. Can not recognize mpu6050\n", strlen("who am i error. Can not recognize mpu6050\n"), 100);
			HAL_Delay(100);
		}
	}
	...
	char msg[80];
	sprintf(msg, "LSB_Sensitivity_GYRO: %f, LSB_Sensitivity_ACC: %f\n", LSB_Sensitivity_GYRO, LSB_Sensitivity_ACC);
	HAL_UART_Transmit(&huart2, (uint8_t *)msg, strlen(msg), 100);
