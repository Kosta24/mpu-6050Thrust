...
// #define dataToPc(buf, len) HAL_UART_Transmit_DMA(&huart2, buf, len, HAL_MAX_DELAY)
...
{
	HAL_I2C_Mem_Write(&hi2c1, MPU6050_ADDR << 1, reg_addr, I2C_MEMADD_SIZE_8BIT, &val, 1, 1);
}

void MPU6050_Writebytes(uint8_t reg_addr, uint8_t len, uint8_t *data)
{
	HAL_I2C_Mem_Write(&hi2c1, MPU6050_ADDR << 1, reg_addr, I2C_MEMADD_SIZE_8BIT, data, len, 1);
}

void MPU6050_Readbyte(uint8_t reg_addr, uint8_t *data)
{
	HAL_I2C_Mem_Read(&hi2c1, MPU6050_ADDR << 1, reg_addr, I2C_MEMADD_SIZE_8BIT, data, 1, 100);
}

void MPU6050_Readbytes(uint8_t reg_addr, uint8_t len, uint8_t *data)
{
	HAL_I2C_Mem_Read(&hi2c1, MPU6050_ADDR << 1, reg_addr, I2C_MEMADD_SIZE_8BIT, data, len, 100);
}
...
HAL_StatusTypeDef MPU6050_Initialization(void)
{
	...
	HAL_UART_Transmit_DMA(&huart2, (uint8_t *)"Checking MPU6050...\n", 20);

	if (HAL_I2C_IsDeviceReady(&hi2c1, MPU6050_ADDR << 1, 3, 100) != HAL_OK)
	{
		//printf("MPU6050 not found\n")
		HAL_UART_Transmit_DMA(&huart2, (uint8_t *)"MPU6050 not found\n", 19);
	...
	if (who_am_i == 0x68)
	{
		...
		HAL_UART_Transmit_DMA(&huart2, (uint8_t *)"MPU6050 who_am_i = cose\n",25);
	}
	else
	{
		//printf("ERROR!\n");
		...
		HAL_UART_Transmit_DMA(&huart2, (uint8_t *)"who am i error. Can not recognize mpu6050\n",43);
		return HAL_TIMEOUT;
	}
...
{
	//printf("Inside Get_LSB_Sensitivity\n");

	switch (FS_SCALE_GYRO)
	{
	case 0:
		LSB_Sensitivity_GYRO = 131.f;
		//printf("FS_SCALE_GYRO 0: %f\n", LSB_Sensitivity_GYRO);
		break;
	case 1:
		LSB_Sensitivity_GYRO = 65.5f;
		//printf("FS_SCALE_GYRO 1: %f\n", LSB_Sensitivity_GYRO);
		break;
	case 2:
		LSB_Sensitivity_GYRO = 32.8f;
		//printf("FS_SCALE_GYRO 2: %f\n", LSB_Sensitivity_GYRO);
		break;
	case 3:
		LSB_Sensitivity_GYRO = 16.4f;
		//printf("FS_SCALE_GYRO 3: %f\n", LSB_Sensitivity_GYRO);
		break;
	default:
		//printf("Invalid FS_SCALE_GYRO: %d\n", FS_SCALE_GYRO);
		break;
	}
	...
	case 0:
		...
		//printf("FS_SCALE_ACC 0: %f\n", LSB_Sensitivity_ACC);
		break;
	case 1:
		LSB_Sensitivity_ACC = 8192.f;
		//printf("FS_SCALE_ACC 1: %f\n", LSB_Sensitivity_ACC);
		break;
	case 2:
		LSB_Sensitivity_ACC = 4096.f;
		//printf("FS_SCALE_ACC 2: %f\n", LSB_Sensitivity_ACC);
		break;
	case 3:
		LSB_Sensitivity_ACC = 2048.f;
		//printf("FS_SCALE_ACC 3: %f\n", LSB_Sensitivity_ACC);
		break;
	default:
		//printf("Invalid FS_SCALE_ACC: %d\n", FS_SCALE_ACC);
		break;
	}
